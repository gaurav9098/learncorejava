...
public class MethodReferenceExample {
	public static void main(String args[]) {
		...
		List<Order> orderBook = new  ArrayList<>(); 
		 ...
		 // replacing lambda expression to method reference  Above code can also be written like this, where 
		 // we are just calling a method of Order class from  lambda expression, this can be replaced by Method  reference. 
		 ...
		 // for method parameter, it’s just a name  Similarly you can call other static method  using method reference. 
		 // Another key thing is syntax of method must match with syntax of functional interface,
		 // for example compareByQuantity() syntax is same as compare() method of Comparator interface, which is a functional 
		 // interface and Collections.sort() accept Comparator. 
		 //Let's sort this List by trade value 
		 
		 Collections.sort(orderBook, Order::compareByValue);
		 System.out.println("After sorting by trade value : " + orderBook); 
		 
		 // Java supports four types of method reference,  let's see example of each of them 
		 // Our previous example, in which we are referring to static method was an example of static method reference, 
		 // while below is an example of instance method  reference, where we are invoking and	instance/ method from Order class. 
		 // You can	reference a constructor	in the	same way  as a static method by	using the name new	
		 
		 Order order = orderBook.get(0); 
		 
		 // you need a 	reference of object 
		 
		 Collections.sort(orderBook,order::compareByPrice);
		 System.out.println("Order book after sorting 	by price:" + orderBook); 
		 
		 // method reference example of an 	Arbitrary Object of a Particular Type  equivalent lambda expression for following would be 
...
class Order {
	...
	@Override public
		 String toString() { return String.format("%s %d %s at price %.02f",side, quantity, symbol, price); } public static int
